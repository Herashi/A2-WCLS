...
)
{
  ...
  estimating_equation <- function(theta) {
    alpha <- as.matrix(theta[1:q])
    beta <- as.matrix(theta[(q+1):(q+p)])
    mu <- as.matrix(theta[q+p+1])
    
    Xdm[,2] = Xdm[,2] - rep(mu,length.out = nrow(Xdm))
    ...
    exp_AXdm_beta_causal = exp(A * beta[1] * Xdm[,1])
    ...
    ef[p+q+1] <- sum( (exp_AXdm_beta_causal^(-1) - weight) * Y * avail * WCLS_weight * cA_tilde * Xdm[, 1])
  ...
  if (is.null(estimator_initial_value)) {
    estimator_initial_value <- c(-1.3,0.3,0.3464561,0.2,1)
  }
  ...
  alpha_hat <- as.vector(estimator$alpha)
  beta_hat <- as.vector(estimator$beta)
  mu_hat = solution[["root"]][p+q+1]
  
  Xdm[,2] = Xdm[,2] - rep(mu_hat,nrow(Xdm))
  ...
  Mn_summand <- array(NA, dim = c(total_person_decisionpoint, p+q, p+q))
  Sigma_E <- array(NA, dim = c(total_person_decisionpoint, p+q, 1))
  dot_E <- array(NA, dim = c(total_person_decisionpoint, 1, 1))
  phi = array(NA, dim = c(total_person_decisionpoint, p+q , 1))
  
  U_E <- rep(NA, total_person_decisionpoint)
  # Mn_summand is \frac{\partial D^{(t),T}}{\partial \theta^T} r^(t) + D^{(t),T} \frac{\partial r^(t)}{\partial \theta^T}
  # See note 2018.08.06 about small sample correction
  ...
  for (it in 1:total_person_decisionpoint) {
    ...
    pre_multiplier <- exp(- A[it] * Xbeta) * WCLS_weight[it]
    pre_multiplier_causal = exp(- A[it] * Xdm[it, 1] * beta_hat[1]) * WCLS_weight[it] 
    ...
    Mn_summand[it, , ] <- partialD_partialtheta * r_term + D_term %o% partialr_partialtheta
    Sigma_E[it,1:q , ] <- beta_hat[2] * pre_multiplier * A[it] * Y[it] * (Zdm[it, ] %o% Xdm[it,1 ])
    Sigma_E[it,q+1, ] <- beta_hat[2] * pre_multiplier * Y[it]* A[it] * cA_tilde[it] * (Xdm[it,1 ] %o% Xdm[it,1 ])
    Sigma_E[it,q+p, ] <- -r_term* pre_multiplier *  cA_tilde[it] * Xdm[it,1 ] + beta_hat[2] * pre_multiplier * Y[it]* A[it] * cA_tilde[it] * (Xdm[it,2 ] %o% Xdm[it,1 ])
    dot_E[it, ,] <- -beta_hat[2] * pre_multiplier * Y[it]* A[it] * cA_tilde[it] * (Xdm[it,1 ] %o% Xdm[it,1 ])
    
    phi[it,1:q, ] <- 0
    phi[it,q+1, ] <- - (pre_multiplier_causal - pre_multiplier ) * A[it] * Y[it]* cA_tilde[it] * (Xdm[it,1 ] %o% Xdm[it,1 ])
    phi[it,q+p, ] <- pre_multiplier* Y[it]* cA_tilde[it] * A[it] * (Xdm[it,2 ] %o% Xdm[it,1 ])
    
    U_E[it] <- (pre_multiplier_causal - pre_multiplier )* Y[it]* cA_tilde[it]* Xdm[it,1 ]
    }
  Mn <- apply(Mn_summand, c(2,3), sum) / sample_size
  Sigma_E <- apply(Sigma_E, c(2,3), sum) / sample_size
  dot_E <- apply(dot_E, c(2,3), sum) / sample_size
  phi <- apply(phi, c(2,3), sum) / sample_size
  Mn_inv <- solve(Mn - Sigma_E%*%(dot_E)^(-1)%*% t(phi))
  ...
  for (i in 1:sample_size) {
    D_term_i <- D_term_collected[, person_first_index[i] : (person_first_index[i+1] - 1)]
    r_term_i <- matrix(r_term_collected[person_first_index[i] : (person_first_index[i+1] - 1)], ncol = 1)
    u_term_i <- sum(U_E[person_first_index[i] : (person_first_index[i+1] - 1)])
    
    U = D_term_i %*% r_term_i - Sigma_E%*%(dot_E)^(-1)%*% u_term_i
    
    Sigman_summand[i, , ] <- U %*% t(U)
  }
  Sigman <- apply(Sigman_summand, c(2,3), sum) / sample_size
  ...
  return(list(beta_hat = beta_hat, alpha_hat = alpha_hat,mu_hat = mu_hat,
              beta_se = beta_se, alpha_se = alpha_se,
              beta_se_adjusted = beta_se_adjusted, alpha_se_adjusted = alpha_se_adjusted,
              varcov = varcov,
              varcov_adjusted = varcov_adjusted,
