...
rsnmm = function(n, T,
  ...
  for (i in 0:(n-1)) {
    for (j in 2:T) {
      ...
      # overwrite the q vector as a time indicator
      #true effect now is -0.2 + 0.01 *(j-1) 
      q[i*T + j] = j-1 
     
      # mean of the time-varying state variable
      ...
      # state follows a uniform distribution where the variance is also varying over time
